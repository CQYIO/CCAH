...
def compress(train_loader, test_loader, model_I, model_T, database_dataset, test_dataset):
    re_BI = list([])
    re_BT = list([])
    re_L = list([])
    for _, (data_I, data_T, tag, _) in enumerate(train_loader):
        txt_feature, adjacencyMatrix = generate_txt_graph(data_T)
        txt_feature = torch.FloatTensor(txt_feature.numpy())
        _, code_I, _ = model_I(data_I.cuda())
        code_I = torch.sign(code_I)
        re_BI.extend(code_I.cpu().data.numpy())
        code_T, _ = model_T(txt_feature.cuda(), adjacencyMatrix.cuda())
    ...
    qu_BT = list([])
    qu_L = list([])
    for _, (data_I, data_T, tag, _) in enumerate(test_loader):
        txt_feature, adjacencyMatrix = generate_txt_graph(data_T)
        txt_feature = torch.FloatTensor(txt_feature.numpy())
        _, code_I, _,  = model_I(data_I.cuda())
        code_I= torch.sign(code_I)
...
def generate_txt_graph(txt):
    txt_feature = txt
    adj = txt.mm(txt.t())
    adj = torch.sign(adj)
    adj2triad = sp.csr_matrix(adj)
    adj = normalize_adj(adj2triad)
    adj = torch.FloatTensor(np.array(adj.todense()))
    #adjacencyMatrix = sparse_mx_to_torch_sparse_tensor(adj)
